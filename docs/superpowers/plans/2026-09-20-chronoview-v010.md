...
### Task 17 : Horodatages explicites — timezone serveur affichée, bascule vers l'heure locale du navigateur

**Contexte (retour utilisateur du 2026-09-20, test sur pitch-masters)** : les heures sont affichées en `app.timezone` (UTC) sans indication ; un utilisateur à Bruxelles ne sait pas s'il lit l'heure serveur, locale ou base. Exigence : chaque horodatage doit dire dans quelle timezone il est, et l'utilisateur doit pouvoir basculer vers son heure locale.

**Files:**
- Create: `resources/views/partials/time.blade.php`
- Modify: `resources/views/layouts/app.blade.php` (bouton de bascule + légende + JS), `resources/views/dashboard.blade.php`, `resources/views/tasks/index.blade.php`, `resources/views/tasks/show.blade.php`, `resources/views/runs/index.blade.php`, `resources/views/runs/show.blade.php`, `resources/views/partials/heartbeat.blade.php`, `resources/dist/chronoview.css`
- Test: `tests/Feature/TimezoneDisplayTest.php`

**Interfaces:**
- Produces: partial `chronoview::partials.time` — paramètres `$date` (`?CarbonInterface`), rendu `—` si null, sinon
  `<time class="cv-time" datetime="{ISO 8601 avec offset}" data-exact="{Y-m-d H:i:s}" title="{Y-m-d H:i:s} {tz serveur}">{Y-m-d H:i:s}</time>`.
- Produces: attribut `data-server-tz="{{ config('app.timezone') }}"` sur `<body>`, clé `localStorage` `chronoview-tz` ∈ `server|local`, bouton d'en-tête `aria-label="Toggle timezone"` avec libellé `🕒 Server (UTC)` / `🕒 Local (Europe/Brussels)` (nom de zone du navigateur via `Intl.DateTimeFormat().resolvedOptions().timeZone`), légende sous l'en-tête : « Times are shown in the application timezone (UTC). Switch to your local time with the clock button. » qui change avec la bascule.
- Règle : **tous** les `Format::exact(...)` affichés dans les vues passent par le partial ; `Format::ago()` (relatif) est inchangé. Sur la page tâche, le libellé devient `Task timezone: Europe/Brussels` (ou `Task timezone: UTC (application default)` quand la tâche n'a pas de `->timezone()` explicite différent).

- [ ] **Step 1 : Test `tests/Feature/TimezoneDisplayTest.php`**
...
- [ ] **Step 3 : Partial `resources/views/partials/time.blade.php`**
...
- [ ] **Step 4 : Layout — `data-server-tz`, bouton, légende, JS**

Sur `<body>` ajouter `data-server-tz="{{ config('app.timezone') }}"`. Dans `.cv-header-tools`, avant le bouton thème :
...
CSS : `.cv-tz-note { max-width: 1200px; margin: 8px auto -8px; padding: 0 16px; color: var(--cv-muted); font-size: 12.5px; }` et `time.cv-time { font-family: var(--cv-mono); font-size: 12.5px; font-variant-numeric: tabular-nums; }`.

- [ ] **Step 5 : Remplacer chaque `Format::exact(...)` affiché par `@include('chronoview::partials.time', ['date' => …])`**

Fichiers : `dashboard.blade.php` (Up next), `tasks/index.blade.php` (Next run — remplace `{{ $row['next'] ? Format::exact($row['next']) : '—' }}`), `tasks/show.blade.php` (Next, colonnes Expected/Started), `runs/index.blade.php` (Expected/Started), `runs/show.blade.php` (Expected/Started/Finished), `partials/heartbeat.blade.php` (le `title` peut rester `Format::exact`, mais afficher le beat via le partial à côté du relatif). Les attributs `title="{{ Format::exact(...) }}"` peuvent rester (texte d'info-bulle).

Page tâche : remplacer `<span>{{ $task->timezone ?? config('app.timezone') }}</span>` par
...
- [ ] **Step 6 : Lancer toute la suite — attendu PASS** (les tests existants assertent des chaînes `Y-m-d H:i:s` toujours présentes dans le texte du `<time>`)

- [ ] **Step 7 : `vendor/bin/pint && vendor/bin/phpstan analyse`, README** — ajouter dans la section Screens ou FAQ : « **Which timezone are the times in?** The application timezone by default (shown in the header); click the clock button to switch to your browser's local time. Each task also shows its own scheduling timezone. »

- [ ] **Step 8 : Commit**

```bash
git add -A
git commit -m "feat: explicit server timezone on every timestamp with a local-time toggle"
```

---

### Task 18 : Hygiène du dossier d'output `storage/framework/chronoview/`

**Contexte (test manuel pitch-masters, 2026-09-20)** : (1) le dossier créé par `ScheduleInspector::ensureOutputDirectory()` apparaît comme **non suivi** dans le dépôt de l'application hôte (`?? storage/framework/chronoview/`) ; Laravel évite cela pour `storage/logs` etc. en y plaçant un `.gitignore` contenant `*` + `!.gitignore`. (2) Quand un run est tué en cours (process interrompu), son fichier `<key>.log` n'est jamais lu ni supprimé et s'accumule.

**Files:**
- Modify: `src/Support/ScheduleInspector.php` (`ensureOutputDirectory()`), `src/Commands/PruneCommand.php`
- Test: `tests/Feature/ScheduleInspectorTest.php` (ajout), `tests/Feature/PruneCommandTest.php` (ajout)

**Interfaces:**
- Produces: `ScheduleInspector::outputDirectory(): string` (public, = `storage_path('framework/chronoview')`), `ensureOutputDirectory()` écrit `.gitignore` (`"*\n!.gitignore\n"`) s'il n'existe pas.
- Produces: `chronoview:prune` supprime les `*.log` du dossier d'output dont la mtime est plus vieille que 24 h et affiche `… and N stale output file(s).` en fin de message.

- [ ] **Step 1 : Tests**

Dans `tests/Feature/ScheduleInspectorTest.php` ajouter :
...
Dans `tests/Feature/PruneCommandTest.php` ajouter :

```php
it('removes stale output files left by interrupted runs', function (): void {
    $dir = app(\Grazulex\ChronoView\Support\ScheduleInspector::class)->outputDirectory();
    \Illuminate\Support\Facades\File::ensureDirectoryExists($dir);
    \Illuminate\Support\Facades\File::put($dir . '/old.log', 'x');
    \Illuminate\Support\Facades\File::put($dir . '/fresh.log', 'y');
    touch($dir . '/old.log', time() - 2 * 86400);

    $this->artisan('chronoview:prune')->expectsOutputToContain('1 stale output file')->assertSuccessful();

    expect(\Illuminate\Support\Facades\File::exists($dir . '/old.log'))->toBeFalse()
        ->and(\Illuminate\Support\Facades\File::exists($dir . '/fresh.log'))->toBeTrue();

    \Illuminate\Support\Facades\File::delete($dir . '/fresh.log');
});
```
...
(`outputPathFor()` et `isOurOutput()` utilisent désormais `outputDirectory()`.)

`PruneCommand::handle()` — injecter `ScheduleInspector $inspector` en plus de `Heartbeat`, puis avant le message final :
...
et ajouter une ligne `$this->components->info(sprintf('Removed %d stale output %s.', $files, $files === 1 ? 'file' : 'files'));` (une `info()` séparée, cohérente avec la décision de la Task 10 sur `expectsOutputToContain`).

- [ ] **Step 4 : Lancer la suite — attendu PASS ; Pint ; PHPStan**

- [ ] **Step 5 : README** — dans « How it works » ou FAQ, une phrase : « Command output is captured in `storage/framework/chronoview/` (git-ignored automatically); `chronoview:prune` removes files older than a day left by interrupted runs. »

- [ ] **Step 6 : Commit**

```bash
git add -A
git commit -m "fix: git-ignore the output directory and prune stale output files"
```

---

### Task 19 : Liste d'emails autorisés en configuration (façon Horizon, sans code dans l'app)

**Contexte (demande utilisateur, 2026-09-20)** : « on doit être identifié et on ajoute la liste des emails qui ont accès au dashboard dans une config ».

**Files:**
- Modify: `config/chronoview.php`, `src/ChronoView.php`, `src/Facades/ChronoView.php` (docblock inchangé), `README.md`, `CHANGELOG.md`
- Test: `tests/Feature/AuthorizeTest.php` (ajouts)

**Interfaces:**
- Config : `'auth' => ['allowed_emails' => env('CHRONOVIEW_ALLOWED_EMAILS'), 'guard' => env('CHRONOVIEW_GUARD')]` — `allowed_emails` accepte une chaîne séparée par des virgules (env) **ou** un tableau (config publiée) ; `guard` null = guard par défaut.
- `ChronoView::allowedEmails(): array<int, string>` (normalisées : trim, minuscules, vides retirées).
- Précédence de `ChronoView::check(Request $request): bool` :
  1. callback `auth()` si défini ;
  2. gate `viewChronoView` si défini ;
  3. si `allowedEmails()` non vide : `Auth::guard(config('chronoview.auth.guard'))->user()` doit exister et son `email` (minuscules) être dans la liste ; sinon `false` — **même en `local`** ;
  4. sinon `app()->environment('local')`.

- [ ] **Step 1 : Tests (ajout dans `tests/Feature/AuthorizeTest.php`)**
...
it('allows listed emails and denies the others', function (): void {
    ...
    $admin = new \Illuminate\Foundation\Auth\User(['email' => 'jms@grazulex.be']);
    $other = new \Illuminate\Foundation\Auth\User(['email' => 'someone@example.com']);
...
it('accepts an array of emails from a published config', function (): void {
    ...
    expect(\Grazulex\ChronoView\Facades\ChronoView::allowedEmails())->toBe(['jms@grazulex.be']);
    $this->actingAs(new \Illuminate\Foundation\Auth\User(['email' => 'jms@grazulex.be']))->get('/chronoview')->assertOk();
});

it('lets the callback and the gate take precedence over the allow-list', function (): void {
    config()->set('chronoview.auth.allowed_emails', 'jms@grazulex.be');
    \Grazulex\ChronoView\Facades\ChronoView::auth(fn (): bool => true);

    $this->get('/chronoview')->assertOk();
});
```

`Illuminate\Foundation\Auth\User` a `$fillable` vide : dans les tests utiliser `(new User())->forceFill(['email' => …])` si `new User([...])` ne remplit pas l'attribut.

- [ ] **Step 2 : Lancer — attendu FAIL (la liste est ignorée)**

- [ ] **Step 3 : Config + manager**

`config/chronoview.php` — ajouter après `'middleware'` :
...
Façade : ajouter `@method static array<int, string> allowedEmails()`.

- [ ] **Step 4 : Lancer la suite — attendu PASS ; Pint ; PHPStan**

- [ ] **Step 5 : README** — section « Authorization » (remplacer le paragraphe d'installation qui parle de `ChronoView::auth`) :
...
CHANGELOG (0.1.0 → Features) : « email allow-list authorization (`CHRONOVIEW_ALLOWED_EMAILS`, `CHRONOVIEW_GUARD`) ».

- [ ] **Step 6 : Commit**

```bash
git add -A
git commit -m "feat: email allow-list authorization for the dashboard"
```

---

### Task 20 : Description et fichier source d'une tâche

**Contexte (demande utilisateur, 2026-09-20)** : la page tâche doit afficher la description et le fichier PHP qui contient la commande / la closure.

**Files:**
- Modify: `database/migrations/2026_01_01_000000_create_chronoview_tables.php` (colonne `source` nullable après `description`), `src/Models/MonitoredTask.php` (`@property ?string $source`), `src/Support/TaskDefinition.php`, `resources/views/tasks/show.blade.php`, `resources/views/tasks/index.blade.php`, `README.md`, `CHANGELOG.md`
- Test: `tests/Unit/TaskDefinitionTest.php` (ajouts), `tests/Feature/TasksPageTest.php` (ajouts)

**Interfaces:**
- `TaskDefinition` gagne `public ?string $source` (constructeur, `toArray()['source']`) — **pas** dans `key()` (l'identité ne change pas).
- `TaskDefinition::sourceOf(Event $event): ?string` :
  - `CallbackEvent` avec closure → `fichier:ligne` (relatif à `base_path()` si dessous, sinon chemin absolu) — **toujours**, même si la closure est nommée ;
  - `CallbackEvent` job (description = classe existante) → fichier de la classe (`ReflectionClass::getFileName()`), relatif ;
  - `Event` artisan → nom de commande normalisé (`artisan foo:bar --x` → `foo:bar`), classe résolue via `Artisan::all()` (`Illuminate\Contracts\Console\Kernel::all()`), fichier relatif ; si la commande est introuvable (ex. closure command `Artisan::command()`) → `null` ;
  - `exec` → `null`.
  - Toute exception de réflexion/résolution → `null` (jamais bloquant).
- Vue `tasks/show` : dans l'en-tête, après la ligne commande : `<div class="cv-meta"><span>Description: …</span></div>` si `description` non nulle **et** différente du nom ; `<span>Source: <code>{{ $task->source }}</code></span>` si non nul. Vue `tasks/index` : `title="{{ $row['task']->source }}"` sur le lien du nom.

- [ ] **Step 1 : Tests**

`tests/Unit/TaskDefinitionTest.php` — ajouter :
...
it('records the class file of an artisan command', function (): void {
    ...
    expect(TaskDefinition::fromEvent($event)->source)->toEndWith('Illuminate/Foundation/Console/InspireCommand.php');
});
...
`tests/Feature/TasksPageTest.php` — ajouter :
...
Migration : `$table->string('source', 500)->nullable();` après `description`.
    ...
    try {
        if ($event instanceof CallbackEvent) {
            ...
            if ($callback instanceof Closure) {
                ...
                return self::relative((string) $r->getFileName()) . ':' . $r->getStartLine();
            }
            $class = is_string($event->description) && class_exists($event->description) ? $event->description : (is_object($callback) ? $callback::class : null);
            return $class !== null ? self::relative((string) (new ReflectionClass($class))->getFileName()) : null;
        }
        ...
        $name = strtok(trim(str_replace(['artisan', Application::artisanBinary(), Application::phpBinary(), php_binary(), artisan_binary()], '', $raw)), ' ');
        $commands = app(ConsoleKernel::class)->all();
        if ($name === false || ! isset($commands[$name])) {
            return null;
        }
...
(`Illuminate\Contracts\Console\Kernel as ConsoleKernel`, `ReflectionClass`, `Throwable` à importer ; `describeCallback()` peut réutiliser `relative()`.) `fromEvent()` passe `source: self::sourceOf($event)` ; `toArray()` ajoute `'source' => $this->source`.

Vues : voir Interfaces.

- [ ] **Step 4 : Lancer la suite — attendu PASS ; Pint ; PHPStan**

- [ ] **Step 5 : README** (Screens / Task) : « The task page shows the description and the PHP file (and line, for closures) that defines the task. » CHANGELOG : « task description and source file on the task page ».

- [ ] **Step 6 : Commit**

```bash
git add -A
git commit -m "feat: show the description and the source file of a task"
```
